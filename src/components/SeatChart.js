...
const SeatChart = ({ occasion, ticketConcert, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await ticketConcert.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await ticketConcert.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
